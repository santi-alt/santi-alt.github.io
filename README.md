<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Mi primera página web</title>
</head>
<body style="background-color: yellow;">
 <h1 style="font-family:'Courier New', Courier, monospace">Esta es mi primera página web</h1>
 <p>Esta el la primera página web que estoy creando y que estaré modificando constantemente.</p>
 <h2 style="font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">Si quiere más información, <i><ins>ingrese</ins></i> su correo, contraseña y fecha de nacimiento:</h2>
  <form>
    <h1 style="font-family: 'Courier New', Courier, monospace;">Correo:</h1>
    <input type="email" required placeholder="Escribe tu correo aquí" style="font-family: 'Courier New', Courier, monospace;">
    <hr>
    <h1 style="font-family:'Courier New', Courier, monospace">Contraseña:</h1>
    <input type="password" required placeholder="Escribe tu contraseña aquí" style="font-family: 'Courier New', Courier, monospace;">
    <hr>
    <h1 style="font-family: 'Courier New', Courier, monospace;">Fecha de nacimiento:</h1>
    <input type="date" required style="font-family: 'Courier New', Courier, monospace;">
  <hr>
  <input type="submit" style="font-family: 'Courier New', Courier, monospace;">
  </form>
</body>
</html>
